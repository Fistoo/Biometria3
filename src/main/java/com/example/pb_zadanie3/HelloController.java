...
public class HelloController {
    ...
    @FXML
    private ImageView sauvolaImage;

    @FXML
    private ImageView phansalkarImage;
    ...
    public void Sauvola(int window, double k1) throws IOException{
        BufferedImage imageO = ImageIO.read(selectedFile);
        BufferedImage img = ImageIO.read(selectedFile);
        ...
        for (int column = 0; column < width; column++) {
            for (int row = 0; row < height; row++) {
                ...
                Sauvola = mean * (1+0.5*((standardDeviation/128)));
        ...
        sauvolaImage.setImage(convertToFxImage(img));
        sauvolaImage.setFitHeight(150);
        sauvolaImage.setFitWidth(200);
    }

    public void Phansalkar(int window, double k1) throws IOException{
        BufferedImage imageO = ImageIO.read(selectedFile);
        BufferedImage img = ImageIO.read(selectedFile);
        ...
        for (int column = 0; column < width; column++) {
            for (int row = 0; row < height; row++) {
                ...
                Panzerfaust = mean*(1+3*Math.exp(-10*mean)+0.25*((standardDeviation/0.5)-1));

                if(pixelA > Panzerfaust)
                    img.setRGB(column, row, 0xffffffff);
                else
                    img.setRGB(column, row, 0xff000000);
            }
        ...
        phansalkarImage.setImage(convertToFxImage(img));
        phansalkarImage.setFitHeight(150);
        phansalkarImage.setFitWidth(200);
    }
